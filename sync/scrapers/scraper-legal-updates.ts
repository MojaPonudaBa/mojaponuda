 ...
 *   - Službeni glasnik BiH: https://www.sluzbenilist.ba
...
async function scrapeAjnLaws(): Promise<LegalScraperResult> {
  ...
  try {
    ...
    const links = extractLinks(html, AJN_BASE, /zakon|pravilnik|uputstvo|odluka/i).slice(0, 10);
...
function extractTags(text: string): string[] {
  ...
  if (/izmjena|dopuna/i.test(text)) tags.push("izmjena");
